...
import '../services/license_plate_detector.dart';
import '../services/model_service_manager_optimized.dart';
...
enum ScanPhase { loading, scanning, capturing, recognizing, result, error }
...
///
/// Manages camera lifecycle, permissions, model loading, and the full scan
/// flow internally. Exposes results via [onPlateRecognized] callback.
///
/// ```dart
/// AnprScannerWidget(
...
///     print(result.fullPlate);
///     Navigator.pop(context);
///   },
/// )
/// ```
class AnprScannerWidget extends StatefulWidget {
  /// Flutter asset path for the detection TFLite model.
  final String detModelPath;

  /// Flutter asset path for the OCR TFLite model.
  final String ocrModelPath;

  /// Called once when a plate has been successfully read.
  final void Function(LicensePlateResult result)? onPlateRecognized;

  /// Called when an unrecoverable error occurs (e.g. camera/model failure).
  final void Function(String error)? onError;

  /// Override default hardware delegate.
  final DelegateType delegateType;

  /// Override default stability / quality gates.
  final CaptureQualityConfig? captureQualityConfig;

  /// Provide an already-loaded detector to skip the loading phase.
  final LicensePlateDetector? preloadedDetector;

  const AnprScannerWidget({
    super.key,
    required this.detModelPath,
    required this.ocrModelPath,
    this.onPlateRecognized,
    this.onError,
    this.delegateType = DelegateType.gpu,
    this.captureQualityConfig,
    this.preloadedDetector,
  });

  @override
  State<AnprScannerWidget> createState() => _AnprScannerWidgetState();
}

class _AnprScannerWidgetState extends State<AnprScannerWidget>
  ...
  Future<void> _initializeModel() async {
    ...
    try {
      ...
      _detector = await ModelServiceManagerOptimized().getOrLoadDetector(
        widget.detModelPath,
        widget.ocrModelPath,
        delegateType: widget.delegateType,
        onStatusChange: (s) {
          if (mounted && !_isDisposed) {
            setState(() {
              _loadingProgress = s.progress;
              _loadingMessage = s.message;
            });
          }
        },
      );
  ...
  void _startScanning() {
    ...
    _processor = DetectionOnlyProcessor(
      ...
      qualityConfig: widget.captureQualityConfig ??
  ...
  Future<void> _onCaptureReady(CapturedFrame frame) async {
    ...
    await _stopStream();
    // Flash widget calls _onFlashComplete via its onComplete callback.
  }
  ...
  Future<void> _runOcr() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        widget.onPlateRecognized?.call(result);
      }
    } catch (e) {
  ...
  Widget _searchingIndicator() => Container(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 10),
            Text(
              'Point camera at a license plate',
              style: TextStyle(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              const Text(
                'Please grant camera access to scan license plates.',
